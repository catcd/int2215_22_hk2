...
class ThreeDCNN:
    ...
    def _add_logits_op(self):
        ...
        if self.model_variant == 'NORMAL':
            ...
            cnn_input = tf.stack([final_we, final_de], axis=-2)
            cnn_input = tf.reshape(cnn_input, shape=[-1, 3, self.max_seq_len*2, max(dim_dependency_final, dim_word_final)])
        else:
        ...
        with tf.variable_scope("conv"):
            ...
            for k in self.cnn_filters:
                with tf.variable_scope("cnn-{}".format(k)):
                    ...
                    cnn_op = tf.layers.conv3d(
                        cnn_input, filters=filters,
                        kernel_size=(1, height, cnn_filter_width),
                        strides=(1, cnn_step, 1),
                        padding="valid", name='cnn-{}'.format(k),
                        kernel_initializer=tf.contrib.layers.xavier_initializer(),
                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-4)
                    )
                    cnn_op = self.batch_normalization(cnn_op, training=self.is_training)
                    cnn_op = tf.reduce_max(cnn_op, [1, 2, 3])
                    cnn_op = tf.reshape(cnn_op, [-1, filters])
        ...
        with tf.variable_scope("logit"):
            ...
            self.predict = tf.nn.softmax(self.batch_normalization(self.logits, training=self.is_training))
    ...
    def _next_batch(self, data, num_batch):
        ...
        while idx < num_batch:
            ...
            word_ids, _ = pad_sequences(w_batch, pad_tok=0, max_sent_length=self.k, nlevels=2)
            relation_ids, _ = pad_sequences(r_batch, pad_tok=0, max_sent_length=self.k, nlevels=2)
            direction_ids, _ = pad_sequences(d_batch, pad_tok=0, max_sent_length=self.k, nlevels=2)
    ...
    def _train(self, epochs, early_stopping=True, patience=10, verbose=True):
        ...
        with tf.Session() as sess:
            ...
            num_batch = ceil(len(self.dataset_train.words)/self.batch_size)
            ...
            for e in range(epochs):
                ...
                # stop by F1
                if early_stopping:
                    num_batch = len(self.dataset_validation.words) // self.batch_size + 1
                    total_acc = []
                    total_f1 = []
    ...
    # test
    def predict(self, test, predict_class=True):
        """
